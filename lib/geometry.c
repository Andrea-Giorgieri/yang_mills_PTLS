...
void init_rect(Rectangle *most_update, int const defect_level, int const L_R, GParam const * const param)
	...
	int i, j, err;
	long V;
	// rectangle sizes and min and max of rect coordinates along every direction
	V = 1;
	for(i=0; i<STDIM; i++) 
		{
	...
	for(i=0; i<STDIM; i++) most_update->d_size_rect[i] = aux_L[i];
