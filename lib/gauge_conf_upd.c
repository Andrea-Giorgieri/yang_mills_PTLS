...
void calcstaples_with_topo(Gauge_Conf const * const GC,
	...
	const double coeff=(param->d_theta)*((double) NCOLOR)/(GC->B[r]*128.0*PI*PI);
	long k;
	int j, l;
	int i0, j0;
...
//
		...
		// twist condition (clockwise plaquette) and defect modification
		factor=GC->Z[r][dirs_to_si(i,j)];		//Z_\mu\nu(x)
...
//
		...
		// twist condition (anticlockwise plaquette) and defect modification
		factor=GC->Z[k][dirs_to_si(j,i)];	//Z_\nu\mu(x-\nu) = conj(Z_\mu\nu(x-\nu))
...
//
	...
	// twist condition (clockwise plaquette) and defect
	factor=GC->Z[r][dirs_to_si(i,j)];		//Z_\mu\nu(x)
	factor*=GC->B[r]/param->d_beta;
...
//
	...
	// twist condition (anticlockwise plaquette) and defect
	factor=GC->Z[k][dirs_to_si(j,i)];	//Z_\nu\mu(x-\nu) = conj(Z_\mu\nu(x-\nu))
	factor*=GC->B[k]/param->d_beta;
